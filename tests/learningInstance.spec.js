...
test.describe("Use Case 2 — User Defined Learning Instance", () => {
    ...
    test("should create a learning instance with fields, rule, and document", async ({ page }) => {
        ...
        const instanceName = generateUniqueName(automationData.learningInstance.namePrefix);
        const documentPath = resolveTestDataFile(automationData.learningInstance.documentFile);

        await loginPage.navigate();
        await dashboardPage.verifyDashboardLoaded();

        await dashboardPage.openAI();
        await learningInstancePage.navigateToLearningInstances();
        ...
        await learningInstancePage.addFormFields(automationData.learningInstance.formFields);
        await learningInstancePage.addTableFields(automationData.learningInstance.tableFields);
        await learningInstancePage.createFieldRule(automationData.learningInstance.rule);
